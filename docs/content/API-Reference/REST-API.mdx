...
<InfoBox>Require `data` permission.</InfoBox>
...
<InfoBox>Require `meta` permission.</InfoBox>
...
<InfoBox>Require `jobs` permission.</InfoBox>
...
### <--{"id" : "API Reference"}--> /v1/pre-aggregations/jobs
...
Returns the ready state of the deployment. Does not require any permission.
...
Returns the liveness state of the deployment. Does not require any permission.
This is confirmed by testing any existing connections to `dataSource`. If no
connections exist, it will report as successful.
